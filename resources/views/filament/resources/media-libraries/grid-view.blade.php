<x-filament-panels::page>
    <div class="space-y-4">
        <!-- Debug Info -->
        <div class="p-4 bg-blue-100 border border-blue-300 rounded mb-4">
            <p><strong>Grid View Loaded!</strong></p>
            <p>Records count: {{ $records->count() ?? 0 }}</p>
            <p>Records variable exists: {{ isset($records) ? 'Yes' : 'No' }}</p>
        </div>
        ...
        <!-- Grid View -->
        @php
            $records = $records ?? $this->records ?? collect();
        @endphp
        @if($records->count() > 0)
        ...
        @else
            <!-- Empty State -->
            <div class="fi-section rounded-xl bg-white dark:bg-gray-900 p-12 shadow-sm ring-1 ring-gray-950/5 dark:ring-white/10 text-center">
